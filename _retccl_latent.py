...
class ResNetModel(pl.LightningModule):
    def __init__(self, latents_path, num_classes=4, dropout=True,
        ...
        fc.extend([nn.Linear(size[1], size[2]), nn.ReLU(), nn.Linear(size[2], num_classes)])
        ...
        print(f"MODEL ARGS: num_classes={num_classes}, latents_path={latents_path}, dropout={dropout}, " +\
...
class ResNetModelDoubleBinary(ResNetModel):
    def __init__(self, see_opposite_class_data="normal", **kwargs):
        ...
        # Replace the last fc layer with going to 2 nodes instead of 4
        size = [2048, 512, 256]
        fc = [nn.Linear(size[0], size[1]), nn.ReLU()]
        if kwargs['dropout']:
            fc.append(nn.Dropout(0.25))
        fc.extend([nn.Linear(size[1], size[2]), nn.ReLU(), nn.Linear(size[2], 2), nn.Sigmoid()])
        self.fc = nn.Sequential(*fc)
...
if __name__ == "__main__":
    ...
    # Resizing arguments
    parser.add_argument("--img_size", type=str, default=4, help="Image size.")
    ...
    parser.add_argument("--keep_scale", type=bool, default=False, help="Whether to resize the images such that each image has the same scale or not.")
    ...
    test_dataset = Dataset(root_dir=DATA_DIR, class_names=P53_CLASS_NAMES,
        size=args.img_size, labels_filename="test", latents_path=args.latents_path, spacing=args.spacing)
    print("Test dataset size: ", len(test_dataset))
    train_dataset = Dataset(root_dir=DATA_DIR, class_names=P53_CLASS_NAMES,
        size=args.img_size, labels_filename="train", latents_path=args.latents_path, spacing=args.spacing,
        data_limit=None)
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(train_dataset)):
        ...
        for run in range(args.num_runs):
            ...
            model = Model(num_classes=len(P53_CLASS_NAMES), lr=args.lr, weight_decay=args.weight_decay,
                                ...
                                latents_path=args.latents_path, dropout=True,
            ...
            model = Model.load_from_checkpoint(checkpoint_callback.best_model_path,
                                                     encoder_model=args.encoder_model,
                                                     num_classes=len(P53_CLASS_NAMES), dropout=True,
